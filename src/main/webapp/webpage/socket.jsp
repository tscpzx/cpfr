...
<script type="text/javascript">
    $(function () {
        ...
        if ('WebSocket' in window) {
            ws = new WebSocket("ws://" + window.location.host + "/ws?device_sn=xxxxxxx232323");
        } else if ('MozWebSocket' in window) {
            ws = new MozWebSocket("ws://" + window.location.host + "/ws");
        } else {
            //如果是低版本的浏览器，则用SockJS这个对象，对应了后台“sockjs/webSocketServer”这个注册器，
            //它就是用来兼容低版本浏览器的
            ws = new SockJS("http://" + window.location.host + "/ws/sockjs");
        }
        ...
        $("#btn2").bind("click", function () {
            ...
            var user_id = "123";
            $.ajax({
                data: "content=" + content + "&device_sn=" + user_id,
                type: "get",
                dataType: 'text',
                async: false,
